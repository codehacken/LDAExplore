...
from processdata.fileops import write_evaluation_matrix_to_file

para_alpha = 'auto'  #1 / num_of_topics  # 'symmetric'(de),'asymmetric','auto'
num_of_words = 20   # Totally 5792 in our dataset
num_of_topics = 20
num_of_passes = 20  # Default is 1, 10 --> much worse
num_of_updates = 1  # Default is 1 in LDA, 50 give too many repeated words, and one-document words after re-ranking
num_of_trunkss = [10, 25, 50, 100, 200, 400, 1000]   # Default 2000, 200 is good. Large-->too many repeated words, and one-document words after re-ranking; Small->less repeated but words are too general meaning
num_of_iterations = 50  # Default is 50
...
evaluation_file = 'server/data/TVCG_evaluation.txt'
...
if __name__ == "__main__":
    ...
    coherence = [ 0 for i in range(7) ]
    perplexity = [ 0 for i in range(7) ]

    for p, num_of_trunks in enumerate(num_of_trunkss):
        # Perform LDA.
        ...
        coherence[p] = sum(top_coherence) / num_of_topics
        perplexity[p] = lda.perplexity(matrix_doc_word, doc_word_index_set)

    write_evaluation_matrix_to_file(coherence, perplexity, evaluation_file)